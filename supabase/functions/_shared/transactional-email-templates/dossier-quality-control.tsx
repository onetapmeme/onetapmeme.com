...
const DossierQualityControl = ({ name = '', ref = 'CS-XXXX', cardName = '' }: Props) => (
  <Html lang="fr" dir="ltr">
    <Head />
    <Preview>Contrôle qualité final — dossier {ref}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={header}><Heading style={h1}>CardSurgery</Heading></Section>
        <Section style={body}>
          <Heading style={h2}>Opération terminée avec succès</Heading>
          <Text style={text}>Bonjour {name || 'collectionneur'},</Text>
          <Text style={text}>
            Opération terminée avec succès. Vos cartes{cardName ? <> (<strong>{cardName}</strong>)</> : null} passent
            actuellement l'étape du contrôle qualité final et seront protégées avant l'expédition.
          </Text>
...
export const template = {
  component: DossierQualityControl,
  subject: (d: Record<string, any>) => `Contrôle qualité final (${d.ref ?? ''}) — CardSurgery`,
  displayName: 'Dossier quality control',
