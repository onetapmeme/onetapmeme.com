...
const DossierRequested = ({ name = '', ref = 'CS-XXXX', packLabel = '', cardName = '' }: Props) => (
  <Html lang="fr" dir="ltr">
    <Head />
    <Preview>Votre demande {ref} a bien été enregistrée</Preview>
    <Body style={main}>
      <Container style={container}>
        ...
        <Section style={body}>
          <Heading style={h2}>Votre demande a bien été enregistrée ✓</Heading>
          <Text style={text}>Bonjour {name || 'collectionneur'},</Text>
          <Text style={text}>
            Merci pour votre confiance. Nous avons bien reçu votre formulaire concernant la carte{' '}
            <strong>{cardName || 'à traiter'}</strong>. Notre équipe vous indiquera très prochainement
            les instructions sécurisées pour nous faire parvenir vos cartes.
          </Text>
          <Section style={refBox}>
            <Text style={refLabel}>Numéro de suivi</Text>
...
export const template = {
  component: DossierRequested,
  subject: (d: Record<string, any>) => `Demande ${d.ref ?? ''} enregistrée — CardSurgery`,
  displayName: 'Dossier requested',
